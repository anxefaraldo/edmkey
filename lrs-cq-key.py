...
def get_key(input_audio_file, output_text_file):
    ...
    if HIGHPASS_CUTOFF is None:
        audio, sr = librosa.load(path=input_audio_file, sr=SAMPLE_RATE, mono=True, duration=FIRST_N_SECS_ONLY,
    ...
    chroma = librosa.feature.chroma_cqt(y=audio, sr=SAMPLE_RATE, hop_length=HOP_SIZE, bins_per_octave=HPCP_SIZE)
    ...
    chroma = essentia.array(chroma) # we need this to use essentia's Key estimator until we port ours to pytho
